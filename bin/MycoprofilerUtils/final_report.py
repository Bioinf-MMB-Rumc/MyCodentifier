...
def fill_html(JSON, sample, outputDir):
    ...
    # create zero depth table
    if "zero_depth" in data.keys():
        ZERODEPTH = pd.DataFrame.from_dict(data['zero_depth'], orient='index')
        if len(ZERODEPTH) != 0:
            ZERODEPTH = ZERODEPTH.drop(['lit', 'position'], axis=1)
        else:
            ZERODEPTH = pd.DataFrame(['NO DATA'])
    else:
        ZERODEPTH = pd.DataFrame(['NO DATA'])
    ...
    template_vars = {
        ...
        "ZERODEPTH": ZERODEPTH.to_html(index=False, header=True)
    }
