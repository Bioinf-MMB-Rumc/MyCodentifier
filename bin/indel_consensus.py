...
def sort_vcf(body_vcf, contig):
    ...
    # If no variants → nothing to sort
    if body_vcf is None or len(body_vcf) == 0:
        return []
    ...
    for c in contig:
        m = id_pattern.search(c)
        if not m:
            continue
        chr_code = m.group(1)
        sub = body_df[body_df[0] == chr_code].copy()
        if sub.empty:
            continue

        # POS naar numeriek voor sortering
        sub[1] = pd.to_numeric(sub[1], errors='coerce')
        sub = sub.dropna(subset=[1])
        sub = sub.sort_values(1, kind='mergesort')  # stabiel

        # Zet POS terug naar string; alle cellen -> string, NaN -> '.'
        sub[1] = sub[1].astype(int).astype(str)
        sub = sub.fillna('.')
        sub = sub.astype(str)
    ...
    out = pd.concat(bodies, ignore_index=True)
    body_list = ['	'.join(row) for row in out.values.tolist()]
    return body_list


def is_empty_vcf(body_lines):
    """Check if VCF body is empty."""
...
def compare(vcf1, vcf2, window):
    ...
    if len(vcf2_lines[1]) == 0:
        new_header, contig = sort_header_lines(vcf1_lines[0], [])
        body_list = sort_vcf(vcf1_lines[1], contig)
